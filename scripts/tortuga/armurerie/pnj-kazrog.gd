...
func _on_area_3d_body_entered(_body: Node3D) -> void:
	if not tags.have(Tags.KAZROG_AT_AUBERGE):
		gui.open_dialog_next(Dialog.pnjSayLeft(self, _TEXT_1)
			.next(Dialog.pnjSay(orco, _TEXT_2)
				.next(Dialog.pnjSayLeft(self, _TEXT_3)
					.next(Dialog.pnjSay(orco, _TEXT_4)
						.next(Dialog.pnjSayLeft(self, _TEXT_5)
							.next(Dialog.pnjSay(orco, _TEXT_6)
								.on_close(func(): gui.doTransition(_goto_auberge))))))))

func _goto_auberge() -> void :
	tags.add(Tags.KAZROG_AT_AUBERGE)

func on_tag_change() -> void:
	visible = not tags.have(Tags.KAZROG_AT_AUBERGE)
	if visible : position.y = 0
	else : position.y = -3

const _TEXT_1 = "Orco rend moi ma hache. Tu sais qu’un orc n'est rien sans sa hache."
